...
| Production commit | `137be01c86434afc6e80dc688e1ed9e435abe11e` — `fix: rank automatic lyrics by valid timing` |
| Production ancestry | Includes U1 `d75789ee74b1a68c6caf02cd062b14b3c59dcd24` and V1 `ca98a9dea34df8f1cb9ce723543eb89ce2ed4595` |
| Test identity during aggregate | Base `ca98a9d` plus the exact production/test diff later committed as `137be01`; no source or test content changed between that aggregate and the production commit |
| Candidate build configuration | Xcode scheme `SpotifyLyrics`, `Debug`, arm64, `CODE_SIGNING_ALLOWED=NO` |
...
- Candidate App: `/tmp/spotifylyrics-nightly-deriveddata.0OfXMh/DerivedData/Build/Products/Debug/SpotifyLyrics.app`
- Source commit: `137be01c86434afc6e80dc688e1ed9e435abe11e`
- Bundle identifier: `com.spotifylyrics.app`
- SHA-256 of `Contents/MacOS/SpotifyLyrics`: `c5607803ea45b4cd63e9d7d6eef1c613dfe42b6a89af2374485cc4795f0de0d4`
- The App is an ordinary temporary Debug build, unsigned, unlaunched, and not archived as a release.
- Xcode briefly registered the temporary product in Launch Services during build; that exact path was unregistered afterward and verified absent (`candidate_registered=NO`).
...
Status remains `USER_VERIFICATION_REQUIRED / NOT_RUN`; no `M1_READY` claim is made.
...
Rollback the production code with a normal revert of `137be01`; this introduces no schema migration. Reverting code cannot undo lyrics versions/selection/timing saved while testing the candidate, nor external player/AI/capture activity. Restore data only under the quiesced backup procedure above.
