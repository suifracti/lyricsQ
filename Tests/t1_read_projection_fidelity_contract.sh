...
TMP_DIR="$(mktemp -d /tmp/spotifylyrics-t1-read-projection.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT
cp "$ROOT_DIR/Tests/t1_read_projection_fidelity_contract.swift" "$TMP_DIR/main.swift"
...
swiftc -D DEBUG -parse-as-library \
  ...
  "$TMP_DIR/main.swift" \
  -o "$TMP_DIR/t1-read-projection-fidelity-contract"

if (($#)); then
  "$TMP_DIR/t1-read-projection-fidelity-contract" "$@"
else
  "$TMP_DIR/t1-read-projection-fidelity-contract" all
fi
