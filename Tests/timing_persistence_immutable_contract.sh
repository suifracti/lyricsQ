...
cd "$ROOT_DIR"
TMP_DIR="$(mktemp -d /tmp/spotifylyrics-timing-sqlite.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT
cp Tests/timing_persistence_immutable_contract.swift "$TMP_DIR/main.swift"
...
swiftc -parse-as-library \
  ...
  "$TMP_DIR/main.swift" \
  -o "$TMP_DIR/timing-persistence-immutable-contract"

"$TMP_DIR/timing-persistence-immutable-contract"
